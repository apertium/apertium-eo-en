#!/bin/bash
 ...
 lt-expand apertium-eo-rn.eo.dixtmp1 | grep -e ':>:' -e '\w:\w' | sed 's/:>:/%/g' | sed 's/:/%/g' | cut -f1 -d'%' > tmp_testvoc0.txt &

 lt-expand apertium-eo-rn.eo.dixtmp1 | grep -e ':>:' -e '\w:\w' | sed 's/:>:/%/g' | sed 's/:/%/g' | cut -f2 -d'%' |  sed 's/^/^/g' | sed 's/$/$ ^;<sent>$/g' | tee tmp_testvoc1.txt |
